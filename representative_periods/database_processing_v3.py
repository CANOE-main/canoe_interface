...
def process_database(database: str, input_path: str = None, output_path: str = None):
    ...
    if not input_path and _get_schema_version(database) != (3, 0): return
    ...
    elif utils.config['days_per_period'] > 1: process_multiday_period(database, hours, db_path)
...
def process_multiday_period(database, hours, db_path):
...
def process_single_day_period(database, hours, input_path, output_path):
