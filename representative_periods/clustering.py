...
def run(show_plots=False):
    ...
    # If not rerunning clustering, try to fetch from cache
    if not utils.config.get('rerun_clustering', True):
        import shutil
        method = utils.config.get('clustering_method', 'hierarchical')
        final_periods = utils.config['final_periods']
        csv_name = f"{method}_{final_periods}p.csv"
        seq_name = f"sequence_{final_periods}p.csv"
        
        periods_path = out_data + "representative_periods/" + csv_name
        sequence_path = out_data + "representative_periods/" + seq_name
        
        if os.path.exists(periods_path) and os.path.exists(sequence_path):
            print(f"Skipping clustering. Loading {final_periods} periods from cache.")
            shutil.copy(periods_path, this_dir + "periods.csv")
            shutil.copy(sequence_path, this_dir + "sequence.csv")
            return
        else:
            print("Cached clustering results not found. Running clustering.")
    ...
    plot_out = os.path.join(dirs.user_config_path, "clustering_plots")
    os.makedirs(os.path.join(plot_out, "duration_curve_plots"), exist_ok=True)
    os.makedirs(os.path.join(plot_out, "timeseries_plots"), exist_ok=True)

    for ts in df_timeseries.columns:
        dur_axes[ts].legend()
        dur_figs[ts].savefig(os.path.join(plot_out, "duration_curve_plots", f"{ts}.pdf"))
        ts_axes[ts].legend()
        ts_figs[ts].savefig(os.path.join(plot_out, "timeseries_plots", f"{ts}.pdf"))
        
        # Close figures to avoid memory leaks
        pp.close(dur_figs[ts])
        pp.close(ts_figs[ts])
...
def cluster_days(df_timeseries: pd.DataFrame, n_periods: int) -> pd.DataFrame:
    ...
    df_sequence.to_csv(out_data + f"representative_periods/sequence_{n_periods}p.csv")
