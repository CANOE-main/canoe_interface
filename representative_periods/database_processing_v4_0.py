...
schema = this_dir + "canoe_schema_v4_0.sql"
...
def process_single_day_period(database: str, hours: list):
    ...
    for i, row in df_sequence.iterrows():
        ...
        curs.execute(f"""REPLACE INTO
                    ...
                    VALUES({i}, '{period_seq}', '{row['period']}', {row['count'] / total_days}, 'Reconstructed original year from clustering')""")
            
    # time_of_day
    for h in range(24):
        tod = f"H0{h+1}" if h+1 < 10 else f"H{h+1}"
        curs.execute(f'REPLACE INTO time_of_day(sequence, tod, hours) VALUES({h+1}, "{tod}", 1)')
